...
const calculateEffectValue = (baseValue: number, empowerLevel: number) => {
    return Number(
        (baseValue * (1 + empowerLevel * EMPOWER_BOOST)).toFixed(2)
    );
}
// #endregion
...
export default function gameContextProvider(this: Card, game: Game, characterIndex: number, messageCache: MessageCache) {
    ...
    const changeStatWithEmpower = (target: Character, stat: StatsEnum, effectIndex: number) =>{
        const empowered = calculateEffectValue(this.effects[effectIndex], this.empowerLevel);
