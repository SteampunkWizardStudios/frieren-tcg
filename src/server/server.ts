...
const jsonPath = '../tcg/characters/characterData/characters/characterStats.json';
...
app.put('/stats/:character', async (req, res) => {
  try {
    ...
    if (ACCEPTED_CHARACTER_INPUT.has(req.params.character)) {
      ...
      res.status(400).json({ error: `Invalid character ${req.params.character}. Character can only be "Serie" or "Sein".` });
    }
  } catch (error) {
    console.log(error);
